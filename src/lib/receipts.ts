...
export const chapters: Chapter[] = CHAPTER_DEFS.map((def, i) => {
  const items = receipts.filter((r) => inChapter(r, def.years));
  const plays = sum(
  ...
  const signatures = [
    ...
    `Peak year · ${plays.toLocaleString()} plays`,
...
function tagOverlap(a: Receipt, b: Receipt) {
  const set = new Set(a.tags);
  return b.tags.filter((t) => t.length > 2 && set.has(t));
}
...
export function findConnections(anchor: Receipt, limit = 8): Link[] {
  ...
  if (anchor.place) {
    for (const r of receipts) {
      if (r.place === anchor.place) consider(r, `Also in ${anchor.place}`, 6);
    }
  }
  ...
  if (anchor.subtitle) {
    for (const r of receipts) {
      if (r.subtitle === anchor.subtitle && r.type === anchor.type) consider(r, `Same ${anchor.type === "music" ? "artist" : "category"}`, 3);
    }
  }

  // shared tags
  for (const r of receipts) {
    const shared = tagOverlap(anchor, r);
...
export const moments: Moment[] = (() => {
  ...
  for (const [day, items] of byDay) {
    ...
    const headline = [
      ...
      place ? `in ${place}` : null,
      spend ? `· ₹${Math.round(spend).toLocaleString()} spent` : null,
    ]
      .filter(Boolean)
      .join(" ");
...
export const patterns: Pattern[] = (() => {
  ...
  return [
    ...
    },
    {
      ...
      value: `${(stats.topArtists[0]?.plays ?? 0).toLocaleString()}×`,
...
export function searchReceipts(
  ...
  return receipts.filter((r) => {
    ...
    if (chapterSlug) {
      const c = chapters.find((x) => x.slug === chapterSlug);
      if (c && !inChapter(r, c.years)) return false;
    }
