...
export function TypeGlyph({ type, className = "" }: { type: Receipt["type"]; className?: string }) {
  const m = TYPE_META[type];
  return (
    <span className={`grid size-8 shrink-0 place-items-center rounded-md font-mono text-sm ${m.bg} ${m.text} ${className}`}>
      {m.glyph}
    </span>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-sm font-medium transition group-hover:text-accent">{receipt.title}</p>
